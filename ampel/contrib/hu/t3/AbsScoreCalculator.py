...
class AbsScoreCalculator(AbsT3ReviewUnit, abstract=True):
    ...
    def process(self, gen: Generator[SnapView, T3Send, None], t3s: None | T3Store = None) -> UBson | UnitResult:
        """
        ...
        for view in gen:
            ...
            for t2unit in self.t2scores_from:
                ...
                scores = [self.evaluate(t2unit, t2_result.body[-1]) for t2_result in view.get_t2_views(unit=t2unit) if isinstance(t2_result.body[-1], dict)]
                if len(scores)==0:
                    continue
