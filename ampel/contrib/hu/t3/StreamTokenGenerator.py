...
class StreamTokenGenerator(AbsT3PlainUnit):
    ...
    cone: None | dict[str, float] = None # supply ra, dec and radius if mode is "cone" 
    ...
    jd_range: list = [2458195., float(Time.now().jd)] # default jd range: [ztf_start, jd_now]
    ...
    def process(self, t3s: T3Store) -> UBson | UnitResult:
        ...
        if self.mode == 'cone':
            if not(self.cone):
                raise TypeError('Missing required argument==> cone={ra:value, dec:value, radius:value}')
            query = {
                    "cone": self.cone,
                    "candidate": candidate
                }
