...
class ScoreTNSObjects(AbsScoreCalculator):
    ...
    maxdist: float = 2. # max squared dist, in arcsec. 
    # TNS prefix has to be amogn these to score
    tns_prefix: list[str] = ['SN']     # use ['SN,'AT'] to also include unclassified transients
    	
    
    # Calculating the score
    powerscale: float = -2. # timediff to tzero to this power gives scaling. 
    ...
    def __init__(self, **kwargs):
        ...
        self.client = TNSClient(
            TNSToken( id=self.tns_id, name= self.tns_name, api_key=self.tns_api_key.value ),
            5,    # timeout
            1,    # parallel 
            self.logger,
        )
    ...
    async def get_tns_discovery(self, ra, dec):
        tdisc = None
        async for doc in self.client.search(ra=ra, dec=dec, radius=self.maxdist, units="arcsec"):
    ...
    def evaluate(
        self, t2unit: str, t2_result: dict[str, Any]
    ) -> float:
        """
