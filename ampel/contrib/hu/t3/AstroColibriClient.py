...
class AstroColibriClient:
    ...
    def store_image(self, image_file: str):
        ...
        payload = json.dumps({"image": im_b64, "file_name": image_file, "other_key": "value"})
...
#                            data=payload, headers=headers, auth=self.session.auth)
    ...
    def firestore_post(self, data: AstroColibriPost, image_path: str | None = None )->Dict[Any,Any]:

        # Upload image file if provided
        if image_path is not None:
            lc_url = self.store_image(image_path)
            self.logger.info('AstroClibri image upload', extra={'local':image_path, 'url':lc_url})
            data["lc_url"] = lc_url
