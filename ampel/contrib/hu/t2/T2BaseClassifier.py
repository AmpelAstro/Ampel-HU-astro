...
def run_parsnip_zsample(sncosmo_table, zs, zweights, model, classifier, delta_zp=0, plot_path=None) -> dict:
    """
    ...
    if np.sum(z_probabilities)>0:
        ...
        result_dict["marginal_lc_classifications"] = dict(zip(types, integrated_probabilities))
...
class BaseClassifier(AmpelBaseModel):
    ...
    def read_class_models(self) -> None:
        ...
        print('read xgb', self.class_xgbmulti)
        self.class_parsnip = {
            label: {
    ...
    def get_multixgb_class(self, classlabel, features):
        ...
        if self.class_xgbmulti[classlabel]['model'].objective=="multi:softmax":
            ...
            print('ran xgb classifier softprob', pvals )
            print('columns', self.class_xgbmulti[classlabel]['columns'] )
    ...
    def get_parsnip_class(self, model_label, lctable, zsample, zweights=None, transient_name='noname') -> dict[str,Any]:
        """
        ...
        if zweights is None:
            zw = [1./len(zsample) for z in zsample] 
        else:
            zw = zweights
            
        ## plot setup, if requested 
        if self.parsnipplot_suffix and self.parsnipplot_dir:
            fname = os.path.join(self.parsnipplot_dir, 't2parsnip_%s_%s.%s'%(transient_name, model_label,self.parsnipplot_suffix))
        else:
            fname = None
                    
        pdict = run_parsnip_zsample(lctable, zsample, zw, 
    ...
    def classify(self, 
        ...
        xgb_multimodels = [],
        parsnip_models = [],
        redshift_weights=None ) -> tuple[list[dict], dict]:
        ...
        if len(flux_table)<4:
            ...
            if len(parsnip_models)>0:
                models = parsnip_models                
            else:
                models = self.class_parsnip.keys()
            if self.add_parsnip_from is not None and not self.add_parsnip_from in models:
                raise ValueError("Feature parsnip model not requested to run.")
            parsnip_class: dict[str,Any] = { model_name: self.get_parsnip_class(model_name, flux_table, redshift_samples, redshift_weights, transient_name) for model_name in models }

        ### Update feature dictionary with parsnip output if requested. 
        if self.add_parsnip_from is not None and not 'Failed' in parsnip_class[self.add_parsnip_from].keys():
...
#            features['parsnip_modelchidof'] = features['parsnip_model_chisq'] / features['parsnip_model_dof']
            ...
            if len( (duplicate := {key: features[key] for key in features if key in parsnip_class[self.add_parsnip_from]['prediction']}) )>0:
                print('Found duplicate keys, cannot safely merge feature dictionaries: ', duplicate)
                raise ValueError( 'Found duplicate keys, cannot safely merge feature dictionaries: ', duplicate) 
            features.update( parsnip_class[self.add_parsnip_from]['prediction'] )
            features['parsnip_modelchidof'] = features['model_chisq'] / features['model_dof']
            print('parsnip feature', parsnip_class[self.add_parsnip_from]['prediction'] )
            print('new features', features.keys() )
            
        ### Run binary XGB classifiers
        if len(xgb_binarymodels)>0:
            models = xgb_binarymodels
        else:
            models = self.class_xgbbinary.keys()
        binary_class: dict[str,Any] = { model_name: self.get_xgb_class(model_name, features) for model_name in models }    

        ### Run multi XGB classifiers
        if len(xgb_multimodels)>0:
            models = xgb_multimodels
        else:
            models = self.class_xgbmulti.keys()
        multi_class: dict[str,Any] = { model_name: self.get_multixgb_class(model_name, features) for model_name in models }    
