#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : ampel/contrib/hu/t2/T2RunLightcurveFitter.py
...
class T2RunLightcurveFitter(T2DigestRedshifts, AbsTabulatedT2Unit):
    """
    ...
    The actual demo fit includes here fits a third order polynomial to the restframe flux

    """

    # Adding default redshift selection values, corresponding to usage of "good" redshifts from catalogs
    redshift_kind: None | str = "T2DigestRedshifts"
    ...
    def process(
        ...
        # All data assembled, carry out ao fit for each redshift return in 'z' (all bands combined...)
        t2_output["fitresults"] = {
            str(z): list(
                np.polyfit(
                    sncosmo_table["time"] / (1 + z), sncosmo_table["flux"], deg=3
                )
            )
            for z in zlist
        }
