...
from ampel.contrib.hu.t2.T2Elasticc2Classifier import (
    BaseElasticc2Classifier,
    add_bts_rateprior,
    add_elasticc2_galcolprior,
)
...
class T2ElasticcAllClassifier(BaseElasticc2Classifier):
    ...
    # Ping limit (SNIa)
    pinglim: float = 0.5
    pingtrange: list[float] = [15.0, 60.0]
    pingmaxmag: float = 22.0
    ...
    def classify(
        ...
        if "Failed" in parsnip_class:
            xgb_prob = self.get_multixgb_class("all", features)
        else:
            # Add parsnip features to features
            features.update(
                {
                    "parsnip_" + featname: featval
                    for featname, featval in parsnip_class["prediction"].items()
                }
            )
            features["parsnip_modelchidof"] = (
                features["parsnip_model_chisq"] / features["parsnip_model_dof"]
            )
        ...
        if features["ndet"] == 0:
            multixgb_class["classifications"].append(
        ...
        all_post_class = add_elasticc2_galcolprior(
            all_post_class, features.get("hostgal_mag_u-g"), features.get("mwebv")
        )
        ...
        snprob = [
            classp["probability"]
            for classp in multixgb_class["classifications"]
            if classp["classId"] == 2222
        ]
        # One should be defined
        if (
            len(snprob) > 0
            and max(snprob) > self.pinglim
            and self.pingtrange[0] <= features.get("t_lc", 0) <= self.pingtrange[1]
        ...
        if "prediction" in parsnip_class:
